...
function pintarSwitch() {
  if (!boton) return;
  // aria-checked mueve la pastilla (todo el estado visual vive en el CSS)
  const etiqueta = activo ? "Ver esta página en español" : "Traducir la página al inglés";
...
export function montarTraductor() {
  boton = document.querySelector(".idioma");
  if (!boton) return;   // el partial del header no trae el switch: nada que hacer
